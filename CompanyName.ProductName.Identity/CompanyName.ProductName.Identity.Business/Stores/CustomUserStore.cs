namespace CompanyName.Identity.Business.Stores
{
        ...
        IDisposable
    {
        ...
        public Task<ApplicationUser> FindByLoginAsync(string loginProvider, string providerKey, CancellationToken cancellationToken)
        {
            ...
            var loginEntity = _UserLoginRepository.Find(new UserLoginKey { LoginProvider = loginProvider, ProviderKey = providerKey });
            if (loginEntity == null)
                return Task.FromResult(default(ApplicationUser));

            var userEntity = _UserRepository.Find(loginEntity.UserId);
        ...
        public Task<IList<string>> GetRolesAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            ...
            IList<string> result = _UserRoleRepository.GetRoleNamesByUserId(user.Id)
                .ToList();
        ...
        public Task<IList<Claim>> GetClaimsAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            ...
            IList<Claim> result = _UserClaimRepository.GetAllByUserId(user.Id)
        ...
        public Task AddClaimsAsync(ApplicationUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken)
        {
            ...
            if (claimEntities.Count() > 0)
            {
                ...
                {
                    _UserClaimRepository.Add(claimEntity, false);
                });

                _UserClaimRepository.SaveChanges();
            }
        ...
        public Task RemoveClaimsAsync(ApplicationUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken)
        {
            ...
            if (claims.Count() > 0)
            {
                claims.ToList().ForEach(claim =>
                {
                    ...
                    _UserClaimRepository.Remove(userClaimEntity.Id, false);
                });
        ...
        public Task RemoveTokenAsync(ApplicationUser user, string loginProvider, string name, CancellationToken cancellationToken)
        {
            ...
            var userTokenEntity = _UserTokenRepository.Find(new UserTokenKey { UserId = user.Id, LoginProvider = loginProvider, Name = name });
        ...
        private UserClaim getUserClaimEntity(Claim value, int userId)
        {
            return value == null
                ? default(UserClaim)
                : new UserClaim
